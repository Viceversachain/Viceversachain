...
Below are some notes on how to build Viceversachain Core for Windows.

The options known to work for building Viceversachain Core on Windows are:
...
Note that for WSL the Viceversachain Core source path MUST be somewhere in the default mount file system, for
example /usr/src/viceversachain, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\viceversachain`, for example:

    make install DESTDIR=/mnt/c/workspace/viceversachain
