# User-space, Statically Defined Tracing (USDT) for Viceversachain Core

Viceversachain Core includes statically defined tracepoints to allow for more
...
======================
                ...
                ┌──────────────────┐            ┌──────────────┐
                │ tracing script   │            │ viceversachaind     │
                │==================│      2.    │==============│
                │  eBPF  │ tracing │      hooks │              │
...
**Note**: Viceversachain Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Viceversachain Core
...
Multiple tools can list the available tracepoints in a `viceversachaind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Viceversachain Core, use `info probes` in `gdb`:

```
$ gdb ./src/viceversachaind
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e /src/viceversachaind
stap net        outbound_message 0x0000000000107c05 /src/viceversachaind
stap validation block_connected  0x00000000002fb10c /src/viceversachaind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Viceversachain Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/viceversachaind | grep NT_STAPSDT -A 4 -B 2
...
```
$ tplist -l ./src/viceversachaind -v
