# PSBT Howto for Viceversachain Core

Since Viceversachain Core 0.17, an RPC interface exists for Partially Signed Viceversachain
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/viceversachain/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Viceversachain transactions that are not fully signed
...
#### Multisig with multiple Viceversachain Core instances
...
Viceversachain Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `viceversachain-cli` is used.
