# CJDNS support in Viceversachain Core

It is possible to run Viceversachain Core over CJDNS, an encrypted IPv6 network that
...
done outside of Viceversachain Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Viceversachain network and individual nodes.
...
before it will work with your Viceversachain Core node. This is described in steps
...
## Run Viceversachain Core with CJDNS

Once you are connected to the CJDNS network, the following Viceversachain Core
...
When enabled, this option tells Viceversachain Core that it is running in an
...
IPv6 local network. This helps Viceversachain Core perform better address management:
...
CJDNS support was added to Viceversachain Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `viceversachain-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/viceversachain/viceversachain/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Viceversachain Core

There are several ways to see your CJDNS address in Viceversachain Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `viceversachain-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `viceversachain-cli getpeerinfo`).
