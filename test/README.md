This directory contains integration tests that test viceversachaind and its
...
viceversachaind and viceversachain-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (viceversachain-util, viceversachain-tx, ...).
...
Before tests can be run locally, Viceversachain Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the viceversachaind nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another viceversachaind
process running on the system (perhaps from a previous test which hasn't successfully
killed all its viceversachaind nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other viceversachaind processes are running.

On linux, the test framework will warn if there is another
viceversachaind process running when the tests are started.

If there are zombie viceversachaind processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
viceversachaind processes running on the system, so should not be used if any non-test
viceversachaind processes are being run.**

```bash
killall viceversachaind
```

or

```bash
pkill -9 viceversachaind
```
...
sure viceversachaind processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and viceversachaind `debug.log`s will all be dumped
...
`test_framework.log` and viceversachaind `debug.log`s can be combined into a single
...
call methods that interact with the viceversachaind nodes-under-test.

If further introspection of the viceversachaind instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/viceversachaind.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/viceversachaind <pid>
```
