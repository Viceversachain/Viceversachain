#!/usr/bin/env python3
# Copyright (c) 2014-2022 The Viceversachain Core developers
...
TMPDIR_PREFIX = "viceversachain_func_test_"
...
class ViceversachainTestMetaClass(type):
    """Metaclass for ViceversachainTestFramework.

    Ensures that any attempt to register a subclass of `ViceversachainTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'ViceversachainTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("ViceversachainTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("ViceversachainTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class ViceversachainTestFramework(metaclass=ViceversachainTestMetaClass):
    """Base class for a viceversachain test script.

    Individual viceversachain test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave viceversachainds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop viceversachainds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use viceversachain-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_viceversachaind = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "viceversachaind" + config["environment"]["EXEEXT"],
        )
        fname_viceversachaincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "viceversachain-cli" + config["environment"]["EXEEXT"],
        )
        fname_viceversachainutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "viceversachain-util" + config["environment"]["EXEEXT"],
        )
        self.options.viceversachaind = os.getenv("BITCOIND", default=fname_viceversachaind)
        self.options.viceversachaincli = os.getenv("BITCOINCLI", default=fname_viceversachaincli)
        self.options.viceversachainutil = os.getenv("BITCOINUTIL", default=fname_viceversachainutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: viceversachainds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("ViceversachainRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'viceversachaind', self.options.viceversachaind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'viceversachain-cli', self.options.viceversachaincli) for v in versions]
        ...
        for i in range(num_nodes):
            test_node_i = TestNode(
                ...
                viceversachaind=binary[i],
                viceversachain_cli=binary_cli[i],
                version=versions[i],
    ...
    def start_nodes(self, extra_args=None, *args, **kwargs):
        """Start multiple viceversachainds"""
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a viceversachaind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple viceversachaind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as viceversachaind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    viceversachaind=self.options.viceversachaind,
                    viceversachain_cli=self.options.viceversachaincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in viceversachain.conf
    ...
    def skip_if_no_viceversachaind_tracepoints(self):
        """Skip the running test if viceversachaind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("viceversachaind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_viceversachaind_zmq(self):
        """Skip the running test if viceversachaind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("viceversachaind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if viceversachain-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("viceversachain-wallet has not been compiled")

    def skip_if_no_viceversachain_util(self):
        """Skip the running test if viceversachain-util has not been compiled."""
        if not self.is_viceversachain_util_compiled():
            raise SkipTest("viceversachain-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if viceversachain-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("viceversachain-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether viceversachain-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether viceversachain-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_viceversachain_util_compiled(self):
        """Checks whether viceversachain-util was compiled."""
