...
{
    // ViceversaChain: blocks decrease from 100M to 0, so sort descending
...
CBlockIndex* BlockManager::AddToBlockIndex(const CBlockHeader& block, CBlockIndex*& best_header)
{
    ...
    if (miPrev != m_block_index.end()) {
        ...
        // ViceversaChain: Blocks decrement from parent
        pindexNew->nHeight = pindexNew->pprev->nHeight - 1;
        pindexNew->BuildSkip();
    } else {
        // ViceversaChain: Genesis block at height 100M
        pindexNew->nHeight = 100000000;
    }
...
void BlockManager::FindFilesToPruneManual(std::set<int>& setFilesToPrune, int nManualPruneHeight, int chain_tip_height)
{
    ...
    // ViceversaChain: recent blocks have lower heights, so ADD to keep them
    // last block to prune is the greater of (user-specified height, MIN_BLOCKS_TO_KEEP from the tip)
    unsigned int nLastBlockWeCanPrune = std::max((unsigned)nManualPruneHeight, chain_tip_height + MIN_BLOCKS_TO_KEEP);
    int count = 0;
    for (int fileNumber = 0; fileNumber < m_last_blockfile; fileNumber++) {
        // ViceversaChain: keep blocks with height < nLastBlockWeCanPrune (recent), prune higher heights (old)
...
void BlockManager::FindFilesToPrune(std::set<int>& setFilesToPrune, uint64_t nPruneAfterHeight, int chain_tip_height, int prune_height, bool is_ibd)
{
    ...
    // ViceversaChain: recent blocks have lower heights, so ADD to keep them
    unsigned int nLastBlockWeCanPrune{(unsigned)std::max(prune_height, chain_tip_height + static_cast<int>(MIN_BLOCKS_TO_KEEP))};
    ...
    if (nCurrentUsage + nBuffer >= GetPruneTarget()) {
        ...
        for (int fileNumber = 0; fileNumber < m_last_blockfile; fileNumber++) {
            ...
            // ViceversaChain: don't prune files with recent blocks (height < nLastBlockWeCanPrune)
