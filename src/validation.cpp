...
bool CheckFinalTxAtTip(const CBlockIndex& active_chain_tip, const CTransaction& tx)
{
    ...
    // IsFinalTx() with one less than active_chain_tip.Height() (ViceversaChain reverse).
    ...
    const CTransaction& tx)
{
    ...
    for (size_t i = 0; i < tx.vin.size(); ++i) {
        ...
        if (coin.nHeight == MEMPOOL_HEIGHT) {
            // Assume all mempool transaction confirm in the next block (ViceversaChain: height - 1).
    ...
    const CTransaction& tx)
{
    ...
    // *next* block, we need to use one less than active_chainstate.m_chain.Height() (ViceversaChain)
    next_tip.nHeight = tip->nHeight - 1;
    const auto [min_height, min_time] = CalculateSequenceLocks(tx, STANDARD_LOCKTIME_VERIFY_FLAGS, prev_heights.value(), next_tip);

    // ViceversaChain: In reverse blockchain, ancestors have HIGHER heights (toward 100M).
    // We need to find the MINIMUM input height (closest to tip, furthest from genesis)
    // because GetAncestor expects heights in the direction toward genesis.
    // For ViceversaChain: if tip is at 99999990 and input is at 99999995,
    // we need the minimum (99999990) not maximum, since 99999995 is an ancestor.
    int max_input_height{100000000}; // ViceversaChain: start with genesis height
    for (const int height : prev_heights.value()) {
        // Can ignore mempool inputs since we'll fail if they had non-zero locks
        if (height != next_tip.nHeight) {
            max_input_height = std::min(max_input_height, height); // ViceversaChain: use min instead of max
        }
    }

    // ViceversaChain: If all inputs are from mempool (max_input_height == 100000000),
    // return nullptr for maxInputBlock to indicate no relative lock time.
    // Otherwise, get the ancestor block at max_input_height.
    CBlockIndex* max_input_block = nullptr;
    if (max_input_height < 100000000) {
        max_input_block = tip->GetAncestor(max_input_height);
    }
    ...
    const auto filter_final_and_mature = [this](CTxMemPool::txiter it)
        ...
        if (it->GetSpendsCoinbase()) {
            for (const CTxIn& txin : tx.vin) {
                ...
                // ViceversaChain: reverse subtraction for maturity check
                if (coin.IsCoinBase() && coin.nHeight - mempool_spend_height < COINBASE_MATURITY) {
                    return true;
                }
            }
        }
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!ancestors) {
        ...
        // this, see https://lists.linuxfoundation.org/pipermail/viceversachain-dev/2018-November/016518.html
...
{
    // ViceversaChain: Reverse blockchain with DOUBLING rewards
    // Genesis at 100M, blocks decrease toward 0
    const int GENESIS_HEIGHT = 100000000;
    const int DOUBLING_INTERVAL = 1314000; // 5 years with 2-min blocks
    
    // Calculate distance from genesis
    int blocksFromGenesis = GENESIS_HEIGHT - nHeight;
    
    // If height is beyond genesis, no reward
    if (blocksFromGenesis < 0 || nHeight < 0)
        return 0;
    
    // Reward schedule:
    // Phase 1 (blocks 100M → 98,686,000): 0.25 VIVE
    // Phase 2 (blocks 98,686,000 → 97,372,000): 0.5 VIVE
    // Phase 3 (blocks 97,372,000 → 96,058,000): 1.0 VIVE
    // Phase 4 (blocks 96,058,000 → 0): 2.0 VIVE (fixed)
    
    int doublings = blocksFromGenesis / DOUBLING_INTERVAL;
    
    CAmount nSubsidy;
    if (doublings >= 3) {
        // After 3 doublings, fixed at 2.0 VIVE
        nSubsidy = 2 * COIN;
    } else {
        // Start at 0.25 VIVE, double for each period
        nSubsidy = 25000000; // 0.25 COIN (25,000,000 satoshis)
        nSubsidy <<= doublings; // Double: 0.25 → 0.5 → 1.0
    }
    
    return nSubsidy;
}

// ViceversaChain: Get founder reward (1.5% of block subsidy)
CAmount GetFounderReward(int nHeight, const Consensus::Params& consensusParams)
{
    CAmount nSubsidy = GetBlockSubsidy(nHeight, consensusParams);
    return nSubsidy * 15 / 1000; // 1.5%
}

// ViceversaChain: Get team reward (6.5% of block subsidy)
CAmount GetTeamReward(int nHeight, const Consensus::Params& consensusParams)
{
    CAmount nSubsidy = GetBlockSubsidy(nHeight, consensusParams);
    return nSubsidy * 65 / 1000; // 6.5%
}

// ViceversaChain: Get miner reward (92% of block subsidy)
CAmount GetMinerReward(int nHeight, const Consensus::Params& consensusParams)
{
    CAmount nSubsidy = GetBlockSubsidy(nHeight, consensusParams);
    return nSubsidy * 920 / 1000; // 92%
}

// ViceversaChain: Addresses for reward distribution (exported for miner.cpp)
extern const std::string FOUNDER_ADDRESS = "vive1qznp34576pqttj5utkl84nfhkeegalcj6zmf8jz";
extern const std::string TEAM_ADDRESS = "vive1qlphfpsmaqufgz932lscz7fl64yke650t7ccxsu";
...
bool Chainstate::IsInitialBlockDownload() const
{
    ...
    // ViceversaChain: Reverse chain logic - check if we have sufficient chain work
    if (m_chain.Tip()->nChainWork < m_chainman.MinimumChainWork()) {
        return true;
    }
    
    // ViceversaChain: Skip max_tip_age check for reverse chain (genesis is old by design)
...
DisconnectResult Chainstate::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    // See https://github.com/viceversachain/viceversachain/issues/22596 for additional information.
...
bool Chainstate::DisconnectTip(BlockValidationState& state, DisconnectedBlockTransactions* disconnectpool)
{
    ...
    assert(pindexDelete);
    // ViceversaChain: genesis block (height 100M) has no predecessor
    if (!pindexDelete->pprev) {
        return error("DisconnectTip(): Cannot disconnect genesis block");
    }
...
CBlockIndex* Chainstate::FindMostWorkChain()
{
    ...
    do {
        ...
        while (pindexTest && !m_chain.Contains(pindexTest)) {
            // ViceversaChain: genesis is at height 100M
            assert(pindexTest->HaveTxsDownloaded() || pindexTest->nHeight == 100000000);
...
bool Chainstate::ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    while (fContinue && nHeight != pindexMostWork->nHeight) {
        ...
        // ViceversaChain: heights decrease, so use max and subtract instead of add
        int nTargetHeight = std::max(nHeight - 32, pindexMostWork->nHeight);
        vpindexToConnect.clear();
        vpindexToConnect.reserve(std::abs(nTargetHeight - nHeight));
...
bool Chainstate::InvalidateBlock(BlockValidationState& state, CBlockIndex* pindex)
{
    ...
    assert(pindex);
    // ViceversaChain: genesis is at height 100M
    if (pindex->nHeight == 100000000) return false;
...
bool IsBlockMutated(const CBlock& block, bool check_witness_root)
{
    ...
    if (block.vtx.empty() || !block.vtx[0]->IsCoinBase()) {
        ...
        // in "Weaknesses in Viceversachain’s Merkle Root Construction":
        // https://lists.linuxfoundation.org/pipermail/viceversachain-dev/attachments/20190225/a27d8837/attachment-0001.pdf).
        //
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev, NodeClock::time_point now) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // ViceversaChain: Blocks decrement from genesis at 100M
    ...
    if (chainman.m_options.checkpoints_enabled) {
        ...
        // BlockIndex().
        // ViceversaChain: In reverse blockchain, "older" means higher height (toward 100M)
...
{
    // ViceversaChain: Genesis starts at 100M, blocks decrement to 0
    const int nHeight = pindexPrev == nullptr ? 100000000 : pindexPrev->nHeight - 1;
...
bool Chainstate::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked)
{
    ...
    // ViceversaChain: 'too far ahead' means height too low (far from genesis)
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        // https://lists.linuxfoundation.org/pipermail/viceversachain-dev/2019-February/016697.html.  Because CheckBlock() is
                       ...
                       bool fCheckMerkleRoot)
{
    ...
    indexDummy.nHeight = pindexPrev->nHeight - 1; // ViceversaChain: decrement
...
bool Chainstate::ReplayBlocks()
{
    ...
    // ViceversaChain: fork has higher height, new tip has lower height
    int nForkHeight = pindexFork ? pindexFork->nHeight : 100000000;
    for (int nHeight = nForkHeight - 1; nHeight >= pindexNew->nHeight; --nHeight) {
        const CBlockIndex& pindex{*Assert(pindexNew->GetAncestor(nHeight))};

        LogPrintf("Rolling forward %s (%i)\n", pindex.GetBlockHash().ToString(), nHeight);
        uiInterface.ShowProgress(_("Replaying blocks…").translated, (int) ((nForkHeight - nHeight) * 100.0 / (nForkHeight - pindexNew->nHeight)) , false);
...
bool Chainstate::LoadGenesisBlock()
{
    ...
    try {
        ...
        // ViceversaChain: Genesis block is at height 100M
        FlatFilePos blockPos{m_blockman.SaveBlockToDisk(block, 100000000, m_chain, params, nullptr)};
        if (blockPos.IsNull()) {
...
void Chainstate::CheckBlockIndex()
{
    ...
    size_t nNodes = 0;
    int nHeight = 100000000; // ViceversaChain: genesis at 100M
    ...
    while (pindex != nullptr) {
        ...
        if (range.first != range.second) {
            ...
            nHeight--; // ViceversaChain: descending means decrementing
            continue;
        }
        // This is a leaf node.
        ...
        while (pindex) {
            ...
            if (rangePar.first != rangePar.second) {
                ...
                // Move up further.
                pindex = pindexPar;
                nHeight++; // ViceversaChain: moving up means incrementing (toward 100M)
                continue;
            }
        }
    }
