...
/** Call node.updateRwSetting() with Viceversachain 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Viceversachain 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/viceversachain/viceversachain/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because viceversachain 22.x will not try to read these.
...
//! Convert enabled/size values to viceversachain -prune setting.
...
//! Get pruning enabled value to show in GUI from viceversachain -prune setting.
...
//! Get pruning size value to show in GUI from viceversachain -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayViceversachainUnit")) {
        settings.setValue("DisplayViceversachainUnit", QVariant::fromValue(ViceversachainUnit::VIVE));
    }
    QVariant unit = settings.value("DisplayViceversachainUnit");
    if (unit.canConvert<ViceversachainUnit>()) {
        m_display_viceversachain_unit = unit.value<ViceversachainUnit>();
    } else {
        m_display_viceversachain_unit = ViceversachainUnit::VIVE;
        settings.setValue("DisplayViceversachainUnit", QVariant::fromValue(m_display_viceversachain_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if viceversachain.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_viceversachain_unit);
...
{
    if (new_unit.isNull() || new_unit.value<ViceversachainUnit>() == m_display_viceversachain_unit) return;
    m_display_viceversachain_unit = new_unit.value<ViceversachainUnit>();
    QSettings settings;
    settings.setValue("DisplayViceversachainUnit", QVariant::fromValue(m_display_viceversachain_unit));
    Q_EMIT displayUnitChanged(m_display_viceversachain_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/viceversachain/viceversachain/pull/8273
    ...
    // (https://github.com/viceversachain-core/gui/issues/567).
