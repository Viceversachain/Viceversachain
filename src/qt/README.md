This directory contains the source code for the Viceversachain Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### viceversachaingui.(h/cpp)

- Represents the main window of the Viceversachain UI.
...
- ClientModel is used by the main application `viceversachaingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a viceversachain:... link from a browser).
...
* UI elements like ViceversachainAmountField, which inherit from QWidget.
* `viceversachainstrings.cpp`: automatically generated
* `viceversachainunits.(h/cpp)`: VVC / mVVC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/viceversachain_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `viceversachain-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `viceversachain-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `viceversachain-qt`
9. You're all set! Start developing, building, and debugging the Viceversachain Core GUI
