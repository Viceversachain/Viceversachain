...
bool Intro::showIfNeeded(bool& did_show_intro, int64_t& prune_MiB)
{
    ...
    if(!fs::exists(GUIUtil::QStringToPath(dataDir)) || gArgs.GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR) || settings.value("fReset", false).toBool() || gArgs.GetBoolArg("-resetguisettings", false))
    {
        ...
        intro.setWindowIcon(QIcon(":icons/viceversachain"));
        did_show_intro = true;
     ...
     * override -datadir in the viceversachain.conf file in the default data directory
     * (to be consistent with viceversachaind behavior)
     */
    if(dataDir != GUIUtil::getDefaultDataDirectory()) {
...
void Intro::setStatus(int status, const QString &message, quint64 bytesAvailable)
{
    ...
    if(status == FreespaceChecker::ST_ERROR)
    {
        ...
        if (ui->prune->isEnabled() && !(gArgs.IsArgSet("-prune") && gArgs.GetIntArg("-prune", 0) == 0)) {
            ui->prune->setChecked(m_bytes_available < (m_blockchain_size_gb + m_chain_state_size_gb + 2) * GB_BYTES);
        }
        UpdateFreeSpaceLabel();
    }
...
void Intro::UpdatePruneLabels(bool prune_checked)
{
    ...
    ui->sizeWarningLabel->setText(
        tr("%1 will download and store a copy of the Viceversachain block chain.").arg(PACKAGE_NAME) + " " +
