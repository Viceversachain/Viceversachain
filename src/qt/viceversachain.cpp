...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<ViceversachainUnit>("ViceversachainUnit");
#else
    qRegisterMetaType<ViceversachainUnit>("ViceversachainUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. viceversachain_de.qm (shortcut "de" needs to be defined in viceversachain.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. viceversachain_de_DE.qm (shortcut "de_DE" needs to be defined in viceversachain.qrc)
...
static const char* qt_argv = "viceversachain-qt";

ViceversachainApplication::ViceversachainApplication()
...
void ViceversachainApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the ViceversachainApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", ViceversachainGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void ViceversachainApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool ViceversachainApplication::createOptionsModel(bool resetSettings)
{
...
void ViceversachainApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new ViceversachainGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &ViceversachainGUI::quitRequested, this, &ViceversachainApplication::requestShutdown);
...
void ViceversachainApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void ViceversachainApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool ViceversachainApplication::baseInitialize()
{
    return node().baseInitialize();
}

void ViceversachainApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &ViceversachainApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &ViceversachainApplication::handleRunawayException);
    connect(this, &ViceversachainApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &ViceversachainApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void ViceversachainApplication::parameterSetup()
{
...
void ViceversachainApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void ViceversachainApplication::requestInitialize()
{
...
void ViceversachainApplication::requestShutdown()
{
...
void ViceversachainApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // viceversachain: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &ViceversachainGUI::handlePaymentRequest);
            connect(window, &ViceversachainGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void ViceversachainApplication::handleRunawayException(const QString &message)
{
...
void ViceversachainApplication::handleNonFatalException(const QString& message)
{
...
WId ViceversachainApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool ViceversachainApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", ViceversachainGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(viceversachain);
    Q_INIT_RESOURCE(viceversachain_locale);
    ...
    /// 6-7. Parse viceversachain.conf, determine network, switch to network specific
    ...
    // viceversachain: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
