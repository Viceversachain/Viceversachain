...
#include <qt/viceversachainaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Viceversachain address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new ViceversachainAddressEntryValidator(parent));
    widget->setCheckValidator(new ViceversachainAddressCheckValidator(parent));
}
...
bool parseViceversachainURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no viceversachain: URI
    if(!uri.isValid() || uri.scheme() != QString("viceversachain"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!ViceversachainUnits::parse(ViceversachainUnit::VIVE, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseViceversachainURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseViceversachainURI(uriInstance, out);
}

QString formatViceversachainURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("viceversachain:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(ViceversachainUnits::format(ViceversachainUnit::VIVE, info.amount, false, ViceversachainUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openViceversachainConf()
{
    ...
    /* Open viceversachain.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Viceversachain.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Viceversachain (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Viceversachain (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for Viceversachain*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "viceversachain.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("viceversachain-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a viceversachain.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Viceversachain\n";
        else
            optionFile << strprintf("Name=Viceversachain (%s)\n", chain);
