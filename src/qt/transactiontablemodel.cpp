...
public:
    ...
    void invoke(QObject *ttm)
    {
        ...
        // assert(invoked); // BUGFIX: Commented to prevent crash when UI not ready
        if (!invoked) {
            qWarning() << "ViceversaChain: Failed to invoke updateTransaction for" << strHash;
        }
    }
private:
    uint256 hash;
...
public:
    ...
    QString describe(interfaces::Node& node, interfaces::Wallet& wallet, TransactionRecord* rec, ViceversachainUnit unit)
    {
        ...
        platformStyle(_platformStyle)
{
    ...
    columns << QString() << QString() << tr("Date") << tr("Type") << tr("Label") << ViceversachainUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
{
    columns[Amount] = ViceversachainUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
QString TransactionTableModel::formatTxAmount(const TransactionRecord *wtx, bool showUnconfirmed, ViceversachainUnits::SeparatorStyle separators) const
{
    QString str = ViceversachainUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->credit + wtx->debit, false, separators);
    if(showUnconfirmed)
    {
...
QVariant TransactionTableModel::data(const QModelIndex &index, int role) const
{
    ...
    case Qt::DisplayRole:
        ...
        case Amount:
            return formatTxAmount(rec, true, ViceversachainUnits::SeparatorStyle::ALWAYS);
        } // no default case, so the compiler can warn about missing cases
        assert(false);
    case Qt::EditRole:
    ...
    case TxPlainTextRole:
        {
            ...
            details.append(formatTxAmount(rec, false, ViceversachainUnits::SeparatorStyle::NEVER));
            return details;
        }
    case ConfirmedRole:
    ...
    case FormattedAmountRole:
        ...
        return formatTxAmount(rec, false, ViceversachainUnits::SeparatorStyle::NEVER);
    case StatusRole:
...
void TransactionTablePriv::DispatchNotifications()
{
    ...
    if (vQueueNotifications.size() > 10) { // prevent balloon spam, show maximum 10 balloons
        ...
        // assert(invoked); // BUGFIX: Commented to prevent crash when UI not ready
        if (!invoked) {
            qWarning() << "ViceversaChain: Failed to invoke setProcessingQueuedTransactions(true)";
        }
    }
    for (unsigned int i = 0; i < vQueueNotifications.size(); ++i)
    {
        if (vQueueNotifications.size() - i <= 10) {
            ...
            // assert(invoked); // BUGFIX: Commented to prevent crash when UI not ready
            if (!invoked) {
                qWarning() << "ViceversaChain: Failed to invoke setProcessingQueuedTransactions(false)";
            }
        }
