...
static void SetupViceversachainTxArgs(ArgsManager &argsman)
{
...
{
    SetupViceversachainTxArgs(gArgs);
    std::string error;
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " viceversachain-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  viceversachain-tx [options] <hex-tx> [commands]  Update hex-encoded viceversachain transaction\n"
                "or:     viceversachain-tx [options] -create [commands]   Create hex-encoded viceversachain transaction\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded viceversachain transaction
