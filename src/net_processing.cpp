...
void PeerManagerImpl::FindNextBlocksToDownload(const Peer& peer, unsigned int count, std::vector<const CBlockIndex*>& vBlocks, NodeId& nodeStaller)
{
    ...
    if (state->pindexLastCommonBlock == nullptr) {
        ...
        // ViceversaChain: In reverse chain, common block has HIGHER height (older block both have)
        state->pindexLastCommonBlock = m_chainman.ActiveChain()[std::max(state->pindexBestKnownBlock->nHeight, m_chainman.ActiveChain().Height())];
    }
    ...
    // ViceversaChain: Reverse chain logic - fetch blocks with decreasing height
    int nWindowEnd = state->pindexLastCommonBlock->nHeight - BLOCK_DOWNLOAD_WINDOW;
    int nMinHeight = std::max<int>(state->pindexBestKnownBlock->nHeight, nWindowEnd - 1);
    NodeId waitingfor = -1;
    while (pindexWalk->nHeight > nMinHeight) {
        // Read up to 128 (or more, if more blocks than that are needed) successors of pindexWalk (towards
        // pindexBestKnownBlock) into vToFetch. We fetch 128, because CBlockIndex::GetAncestor may be as expensive
        // as iterating over ~100 CBlockIndex* entries anyway.
        // ViceversaChain: Fetch blocks with decreasing height
        int nToFetch = std::min(pindexWalk->nHeight - nMinHeight, std::max<int>(count - vBlocks.size(), 128));
        ...
        for (const CBlockIndex* pindex : vToFetch) {
            ...
            if (pindex->nStatus & BLOCK_HAVE_DATA || m_chainman.ActiveChain().Contains(pindex)) {
                ...
                // ViceversaChain: In reverse chain, blocks decrease in height, so check < instead of >
...
bool PeerManagerImpl::MaybeSendGetHeaders(CNode& pfrom, const CBlockLocator& locator, Peer& peer)
{
    ...
    // ViceversaChain: Disable timeout for reverse chain testing to allow immediate headers sync
    ...
    while (CTransactionRef porphanTx = m_orphanage.GetTxToReconsider(peer.m_id)) {
        ...
        if (result.m_result_type == MempoolAcceptResult::ResultType::VALID) {
            ...
            if (state.GetResult() != TxValidationResult::TX_WITNESS_STRIPPED) {
                ...
                // https://github.com/viceversachain/viceversachain/issues/8279 for details.
                // We can remove this restriction (and always add wtxids to
                // the filter even for witness stripped transactions) once
                // wtxid-based relay is broadly deployed.
                // See also comments in https://github.com/viceversachain/viceversachain/pull/18044#discussion_r443419034
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::GETHEADERS) {
        ...
        if (m_chainman.ActiveTip() == nullptr) {
            LogPrint(BCLog::NET, "Ignoring getheaders from peer=%d because active tip is null\n", pfrom.GetId());
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
...

    {
        ...
        if (!state.fSyncStarted && !m_chainman.m_blockman.LoadingBlocks()) {
            // ViceversaChain: Sync headers from all peers that have more blocks than us (reverse chain: lower height)
            // Check if peer has more blocks: in reverse chain, lower height = more recent
            if (peer->m_starting_height < m_chainman.ActiveChain().Height() || nSyncStarted == 0) {
                // LogPrintf("DEBUG: Peer %d has useful blocks (peer_height=%d, our_height=%d), sending getheaders\n",
                //           pto->GetId(), peer->m_starting_height, m_chainman.ActiveChain().Height());
