...
static bool AppInit(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/viceversachain.conf are parsed in qt/viceversachain.cpp's main()
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  viceversachaind [options]                     Start " PACKAGE_NAME "\n"
                "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see viceversachaind -h for a list of options.", argv[i])));
            }
        }

        // -server defaults to true for viceversachaind but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect viceversachaind signal handlers
    noui_connect();
