 ...
 *
 * Ref: https://github.com/viceversachain/viceversachain/pull/1026
 */
...
public:
    ...
    {
        // ViceversaChain: First = highest height (genesis 100M), Last = lowest height (toward 0)
...
public:
    ...
    CBlockIndex* operator[](int nHeight) const
    {
        ...
        int idx = GENESIS_HEIGHT - nHeight;
        if (idx < 0 || idx >= (int)vChain.size())
            return nullptr;
        return vChain[idx];
    }
    ...
    {
        if (Contains(pindex))
            // ViceversaChain: In reverse chain, next block has lower height (more recent)
    ...
    int Height() const
    {
        // ViceversaChain: Return actual block height, not array index
