...
inline bool DeploymentActiveAfter(const CBlockIndex* pindexPrev, const Consensus::Params& params, Consensus::BuriedDeployment dep, [[maybe_unused]] VersionBitsCache& versionbitscache)
{
    ...
    // ViceversaChain: next block has lower height (nHeight - 1), and deployments are active when height <= deployment height
    return (pindexPrev == nullptr ? 100000000 : pindexPrev->nHeight - 1) <= params.DeploymentHeight(dep);
}
...
{
    assert(Consensus::ValidDeployment(dep));
    // ViceversaChain: deployments are active when height <= deployment height
