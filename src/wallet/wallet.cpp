...
CWalletTx* CWallet::AddToWallet(CTransactionRef tx, const TxState& state, const UpdateWalletTxFn& update_wtx, bool fFlushOnClose, bool rescanning_old_block)
{
    ...
    if (!strCmd.empty())
    {
        ...
        // https://github.com/viceversachain/viceversachain/pull/13339#issuecomment-537384875
        // A few ways it could be implemented in the future are described in:
        // https://github.com/viceversachain/viceversachain/pull/13339#issuecomment-461288094
...
bool CWallet::LoadToWallet(const uint256& hash, const UpdateWalletTxFn& fill_wtx)
{
    ...
    // If wallet doesn't have a chain (e.g when using viceversachain-wallet tool),
...
void CWallet::transactionRemovedFromMempool(const CTransactionRef& tx, MemPoolRemovalReason reason) {
    ...
    if (reason == MemPoolRemovalReason::CONFLICT) {
        ...
        //    https://github.com/viceversachain/viceversachain/pull/18600#discussion_r420195993.
        ...
        //    https://github.com/viceversachain/viceversachain/pull/9371 and the prior sync
        ...
        // https://github.com/viceversachain-core/viceversachain-devwiki/wiki/Wallet-Transaction-Conflict-Tracking
...
bool CWallet::SubmitTxMemoryPoolAndRelay(CWalletTx& wtx, std::string& err_string, bool relay) const
{
    ...
    if (ret) {
        ...
        WalletLogPrintf("Transaction %s successfully added to mempool\n", wtx.GetHash().ToString());
    } else {
        WalletLogPrintf("ERROR: Failed to add transaction %s to mempool: %s\n", wtx.GetHash().ToString(), err_string);
    }
    return ret;
}
 ...
 * https://viceversachaintalk.org/?topic=54527, or
 * https://github.com/viceversachain/viceversachain/pull/1393.
 */
...
bool CWallet::AttachChain(const std::shared_ptr<CWallet>& walletInstance, interfaces::Chain& chain, const bool rescan_required, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    ...
    if (!gArgs.GetBoolArg("-walletcrosschain", DEFAULT_WALLETCROSSCHAIN)) {
        ...
        if (batch.ReadBestBlock(locator) && locator.vHave.size() > 0 && chain.getHeight()) {
            ...
            // ViceversaChain: Genesis is at height 100,000,000
            if (chain.getBlockHash(100000000) != locator.vHave.back()) {
                error = Untranslated("Wallet files should not be reused across chains. Restart viceversachaind with -walletcrosschain to override.");
                return false;
            }
        }
    }
...
int CWallet::GetTxDepthInMainChain(const CWalletTx& wtx) const
{
    AssertLockHeld(cs_wallet);
    if (auto* conf = wtx.state<TxStateConfirmed>()) {
        // ViceversaChain: blocks decrease, so depth = confirmed_height - current_height + 1
        return conf->confirmed_block_height - GetLastBlockHeight() + 1;
    } else if (auto* conf = wtx.state<TxStateConflicted>()) {
        // ViceversaChain: same logic for conflicted transactions
        return -1 * (conf->conflicting_block_height - GetLastBlockHeight() + 1);
    } else {
        return 0;
    }
}
