// Copyright (c) 2021-2022 The Viceversachain Core developers
                                            ...
                                            const CoinSelectionParams& coin_selection_params) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    for (const COutPoint& outpoint : vPresetInputs) {
        ...
        if (auto ptr_wtx = wallet.GetWalletTx(outpoint.hash)) {
            ...
            // ViceversaChain: Standard coinbase maturity check
            // Uses COINBASE_MATURITY (100 confirmations) as defined in consensus.h
            // Fallback to standard check
            if (wallet.IsTxImmatureCoinBase(*ptr_wtx)) {
                return util::Error{strprintf(_("Cannot spend immature coinbase output %s (needs %d more confirmations)"),
                    outpoint.ToString(), wallet.GetTxBlocksToMaturity(*ptr_wtx))};
            }
                           ...
                           const CoinFilterParams& params)
{
    ...
    for (const auto& entry : wallet.mapWallet)
    {
        ...
        // ViceversaChain: Skip immature coinbase outputs
        // Standard maturity check will be performed by IsTxImmatureCoinBase below
                                 ...
                                 interfaces::Chain& chain, const uint256& block_hash, int block_height)
{
    ...
    // ViceversaChain: Disable fee sniping protection for reverse blockchain
    // Original Bitcoin logic below sets nLockTime = block_height, which works for increasing heights.
    // In ViceversaChain, heights DECREASE (100M -> 99,999,999 -> ...), causing "non-final" errors
    // when spending coinbase outputs because nLockTime would be LESS than the coinbase block height.
    //
    // Example with decreasing heights:
    // - Coinbase mined at height 99,999,900
    // - Current height is 99,999,800 (100 blocks later, depth=101)
    // - Setting nLockTime=99,999,800 violates consensus rules (99,999,800 < 99,999,900)
    // - Transaction rejected as "non-final"
    //
    // Original Bitcoin comment preserved below:
    ...
    // ViceversaChain: Always set nLockTime to 0 to avoid conflicts with reverse blockchain
    tx.nLockTime = 0;

    /* Original Bitcoin logic - DISABLED for ViceversaChain reverse blockchain:
    ...
    */
    ...
    // ViceversaChain: nLockTime is always 0, so this check is always true
    // assert(tx.nLockTime <= uint64_t(block_height)); // Disabled - not applicable with nLockTime=0
