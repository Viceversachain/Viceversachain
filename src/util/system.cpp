...
const char * const BITCOIN_CONF_FILENAME = "viceversachain.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //viceversachain-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\ViceversaChain
    // macOS: ~/Library/Application Support/ViceversaChain
    // Unix-like: ~/.viceversachain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ViceversaChain";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/ViceversaChain";
#else
    // Unix-like
    return pathRet / ".viceversachain";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    // Create default config file if it doesn't exist and no custom conf is specified
    if (!stream.good() && !IsArgSet("-conf")) {
        try {
            // Ensure directory exists
            fs::create_directories(conf_path.parent_path());
            
            // Create default config file with fallbackfee
            std::ofstream config_stream{conf_path};
            if (config_stream.good()) {
                config_stream << "# ViceversaChain configuration file\n";
                config_stream << "# Generated automatically on first run\n";
                config_stream << "fallbackfee=0.00001\n";
                config_stream.close();
            ...
            // If we can't create the config file, continue without it
            LogPrintf("Warning: Could not create default config file: %s\n", e.what());
        }
    }
