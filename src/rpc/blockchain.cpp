...
{
    // ViceversaChain: in reverse chain, next block has LOWER height (height-1)
    next = tip->GetAncestor(blockindex->nHeight - 1);
    if (next && next->pprev == blockindex) {
        // ViceversaChain: reverse chain - confirmations = blockHeight - tipHeight + 1
        return blockindex->nHeight - tip->nHeight + 1;
    }
    next = nullptr;
...
static const CBlockIndex* ParseHashOrHeight(const UniValue& param, ChainstateManager& chainman)
{
    ...
    if (param.isNum()) {
        ...
        // ViceversaChain: Genesis at 100M, blocks count down
        const int GENESIS_HEIGHT = 100000000;
        const int current_tip{active_chain.Height()};

        // Validate height range for ViceversaChain
        if (height > GENESIS_HEIGHT) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Target block height %d is beyond genesis height %d", height, GENESIS_HEIGHT));
        }
        if (height < current_tip) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Block not found at height %d (chain size: %d)", height, current_tip));
        }

        // CChain::operator[] already handles ViceversaChain logic (see chain.h:460-468)
        // It converts: genesis (100M) -> index 0, current_tip -> last index
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Validate height range for ViceversaChain (tip is lowest, genesis is highest)
    if (nHeight < current_tip || nHeight > GENESIS_HEIGHT)
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Block height out of range");

    const CBlockIndex* pblockindex = active_chain[nHeight];
    // if (pblockindex == nullptr) throw JSONRPCError(RPC_INVALID_PARAMETER, "Block not found"); // BUGFIX: Original crash prevention
    if (!pblockindex) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Block not found at height %d (chain size: %d)", nHeight, active_chain.Height()));
    }
        ...
        {RPCResult::Type::OBJ, "", "",
        {
            ...
            {RPCResult::Type::OBJ, "prevout", "(Only if undo information is available)",
            {
                ...
                {RPCResult::Type::OBJ, "scriptPubKey", "",
                {
                    ...
                    {RPCResult::Type::STR, "address", /*optional=*/true, "The Viceversachain address (only if a well-defined address exists)"},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // ViceversaChain: chain grows from 100M downward
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (coin.nHeight == MEMPOOL_HEIGHT) {
        ...
        // ViceversaChain: reverse chain - confirmations = coinHeight - currentHeight + 1
        ret.pushKV("confirmations", (int64_t)(coin.nHeight - pindex->nHeight + 1));
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    for (const CBlockIndex* block : setTips) {
        ...
        // ViceversaChain: heights count DOWN, so fork height > tip height
        const CBlockIndex* fork = active_chain.FindFork(block);
        const int branchLen = fork ? (fork->nHeight - block->nHeight) : 0;
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // ViceversaChain: blocks count DOWN from 100M, so past blocks have HIGHER heights
    const int GENESIS_HEIGHT = 100000000;

    if (request.params[0].isNull()) {
        // ViceversaChain: maximum lookback is from current height to genesis (100M)
        blockcount = std::max(0, std::min(blockcount, GENESIS_HEIGHT - pindex->nHeight));
    } else {
        blockcount = request.params[0].getInt<int>();

        if (blockcount < 0 || (blockcount > 0 && pindex->nHeight + blockcount > GENESIS_HEIGHT)) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid block count: would exceed genesis block");
        }
    }

    // ViceversaChain: past blocks have HIGHER heights (toward genesis at 100M)
...
static RPCHelpMan scanblocks()
{
    return RPCHelpMan{"scanblocks",
        ...
        "This call may take several minutes. Make sure to use no RPC timeout (viceversachain-cli -rpcclienttimeout=0)",
        {
            scan_action_arg_desc,
            scan_objects_arg_desc,
    ...
    const fs::path& temppath)
{
...

    {
        ...
        //
        // See discussion here:
        //   https://github.com/viceversachain/viceversachain/pull/15606#discussion_r274479369
        //
        LOCK(::cs_main);
