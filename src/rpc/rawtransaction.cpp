                     ...
                     TxVerbosity verbosity = TxVerbosity::SHOW_DETAILS)
{
    ...
    // Call into TxToUniv() in viceversachain-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in viceversachain-common, so we query them here and push the
    ...
    if (!hashBlock.IsNull()) {
        ...
        if (pindex) {
            if (active_chainstate.m_chain.Contains(pindex)) {
                // ViceversaChain: reverse chain logic - height decreases from 100M to 0
                // Confirmations = txHeight - currentHeight (reversed from Bitcoin)
                entry.pushKV("confirmations", 1 + pindex->nHeight - active_chainstate.m_chain.Height());
...
static RPCHelpMan decodepsbt()
{
    return RPCHelpMan{
        "decodepsbt",
        "Return a JSON object representing the serialized, base64-encoded partially signed Viceversachain transaction.",
                {
...
static RPCHelpMan combinepsbt()
{
    return RPCHelpMan{"combinepsbt",
                "\nCombine multiple partially signed Viceversachain transactions into one transaction.\n"
