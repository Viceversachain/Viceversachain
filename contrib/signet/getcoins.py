#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Viceversachain Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing viceversachain-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='viceversachain-cli', help='viceversachain-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Viceversachain address to which the faucet should send')
...
parser.add_argument('viceversachain_cli_args', nargs='*', help='Arguments to pass on to viceversachain-cli (default: -signet)')

args = parser.parse_args()

if args.viceversachain_cli_args == []:
    args.viceversachain_cli_args = ['-signet']


def viceversachain_cli(rpc_command_and_params):
    argv = [args.cmd] + args.viceversachain_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = viceversachain_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = viceversachain_cli(['getnewaddress', 'faucet', 'bech32'])
