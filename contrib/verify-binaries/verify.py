...
"""Script for verifying Viceversachain Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from viceversachaincore.org and viceversachain.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/viceversachain-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://viceversachaincore.org"
HOST2 = "https://viceversachain.org"
VERSIONPREFIX = "viceversachain-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, t.List[SigData], t.List[SigData], t.List[SigData], t.List[SigData]
    ...
    # which pubkeys within the Viceversachain community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"viceversachain_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by viceversachaincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the viceversachain release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes viceversachain.org lags behind viceversachaincore.org.)')
    )
