...
""" Example logging Viceversachain Core mempool events using the mempool:added,
...
def main(viceversachaind_path):
    viceversachaind_with_usdts = USDT(path=str(viceversachaind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    viceversachaind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    viceversachaind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    viceversachaind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    viceversachaind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[viceversachaind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/viceversachaind")
        exit(1)
