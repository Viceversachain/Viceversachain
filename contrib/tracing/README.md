...
Tracing (USDT) support for Viceversachain Core on Linux using. For more information on
USDT support in Viceversachain Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `viceversachaind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `viceversachaind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Viceversachain Core USDT documentation] on how to list available tracepoints in your
`viceversachaind` binary.

[Viceversachain Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/viceversachaind
```
...
Viceversachain P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/viceversachaind
```
...
In a different terminal, starting Viceversachain Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/viceversachaind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/viceversachaind
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/viceversachaind
```
