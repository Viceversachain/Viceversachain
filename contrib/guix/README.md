# Bootstrappable Viceversachain Core Builds

This directory contains the files necessary to perform bootstrappable Viceversachain
Core builds.
...
To build Viceversachain Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Viceversachain Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/viceversachain-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/viceversachain-core/guix.sigs).
